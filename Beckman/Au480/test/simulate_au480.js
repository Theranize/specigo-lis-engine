 ...
 *   Set the integration engine to listen on COM10 (in au480_config.json).
...
function parseArgs() {
  ...
  const options = {
    simPort : '/dev/ttyV1',
    barcode : '69DC9967EB3BB',   // example barcode_uid format from foundation doc
    delayMs : 500,
    runs    : 1,
    scenario: 'standard'
  };
...
// ---------------------------------------------------------------------------
runSimulator().then(() => {
  logger.info('Simulator finished successfully');
   process.exit(0);
}).catch((err) => {
