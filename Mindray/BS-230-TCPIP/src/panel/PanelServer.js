 ...
 *
 * Runs on PANEL_PORT (default 3005 for the BS-230 TCP/IP variant —
 * chosen so it does not collide with the Access 2 panel on 3003 or
 * the BS-230 serial-variant panel on 3004 when multiple engines run
...
class PanelServer {
  ...
  constructor({ engine, port }) {
    ...
    this._engine = engine;
    this._port   = port || parseInt(process.env.PANEL_PORT || '3005', 10);
    this._server = null;
    this._app    = null;
  }
