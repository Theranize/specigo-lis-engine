 ...
 *
 * Implements the ASTM E1394 / LIS2-A2 transmission layer for the Mindray
 * BS-230 chemistry analyser. The byte-level transmission protocol is
 * identical to the Beckman Coulter Access 2 driver - both follow the
 * vanilla LIS2-A2 framing standard. What differs between analyzers is
 * the high-level record content (assay codes, P/O record offsets), which
 * is handled by the per-analyzer parser.
 *
 * Responsibilities:
 ...
 *
 * ASTM E1394 byte-level protocol:
 *
 *   Transmission sequence:
 *     Analyzer sends ENQ (0x05) to initiate.
 *     LIS responds with ACK (0x06).
 *     Analyzer sends one or more frames, each:
 ...
 *     Analyzer sends EOT (0x04) after all messages have been transmitted.
 *
 ...
 *     A single ENQ-EOT transaction may contain results for multiple samples.
 *     Each logical message starts with an H record and ends with an L record.
 *     After the L record's ETX frame the analyzer either sends another H
 *     record frame (next sample) or sends EOT (end of session).
 *
 * Events emitted:
...
class ASTMFramer extends EventEmitter {
   ...
   *                                               "Unidirectional" controls only whether the
   *                                               analyser accepts sample-program downloads
   *                                               from LIS — it does NOT change the ASTM
   *                                               transport-layer framing. ACK/NAK responses
   *                                               are ALWAYS sent (the analyser's ENQ has a
   *                                               ~3s ACK timeout; suppressing ACK causes the
   *                                               analyser to abort the session and drop the
   *                                               TCP connection).
  ...
  constructor(options = {}) {
    ...
    this._unidirectional   = options.unidirectional === true;
    ...
    logger.info('ASTMFramer initialised', {
      ...
      unidirectional : this._unidirectional,
  ...
  _onIdle(byte) {
    ...
    } else if (this._unidirectional && byte === STX) {
      // Unidirectional analysers may skip the ENQ handshake entirely and
      // start the very first frame straight from IDLE. Treat this as an
      // implicit transmission start, fire the same event, and re-process
      // the STX byte through the TRANSMISSION state so the existing frame
      // state machine does the rest.
      this._messageBuffer = '';
      this._state         = STATE.TRANSMISSION;
      logger.info('Implicit transmission start (unidirectional STX in IDLE)');
      this.emit('transmissionStart');
  ...
  _onTransmission(byte) {
    if (byte === STX) {
      ...
      // Re-ENQ: analyzer may send ENQ again after a NAK timeout.
  ...
  _onInFrame(byte) {
    ...
    if (this._frameBody.length > this._maxFrameBytes) {
      ...
      // Return to TRANSMISSION to allow analyzer to retransmit
      this._frameBody = [];
      this._frameNo   = '';
      this._state     = STATE.TRANSMISSION;
    }
  ...
  _sendAck() {
    // ACK is mandatory at the ASTM E1394 transport layer regardless of
    // application-level Unidirectional/Bidirectional mode (manual §8.6.2).
    // The analyser's ENQ has a ~3s ACK timeout — without ACK it aborts the
    // session and tears down the TCP connection.
  ...
  _writeToPort(buffer, description) {
    Promise.resolve()
      ...
      .then(() => {
        logger.info(`${description} sent to analyser`, {
