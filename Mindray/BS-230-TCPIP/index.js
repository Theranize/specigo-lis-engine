/**
 * index.js
 * SpeciGo LIS Integration Engine - Application Entry Point (BS-230 TCP/IP)
 *
 * This is the script that node-windows points to when running as a Windows
 * Service. It is also the script run manually during development and testing:
 *
 *   node index.js
 *   node index.js --config config/analysers/bs230_tcpip_config.json
 *
 * Responsibilities:
 ...
 *   3. Default: config/analysers/bs230_tcpip_config.json
 *
 ...
 *   PANEL_PORT     - Control panel HTTP port (default 3005)
 */
...
const configFilePath = (
  ...
  path.join(process.cwd(), 'config', 'analysers', 'bs230_tcpip_config.json')
);
...
║          Mindray BS-230 (TCP/IP)  |  Clinical Chemistry      ║
...
async function main() {
  ...
  // Start control panel - always start regardless of TCP/DB state.
  try {
    const panel = new PanelServer({ engine });
    await panel.start();
    logger.info('Control panel ready', {
      url: `http://localhost:${process.env.PANEL_PORT || '3005'}`
    });
  } catch (err) {
  ...
  // Start engine - TCP listener and DB connect in background, never blocks here.
  try {
    await engine.start();
    logger.info('Engine started - binding TCP listener and connecting to database in background');
  } catch (err) {
