 ...
 * SpeciGo LIS Integration Engine - Orchestrator (BS-230)
 *
 ...
 *   3. Instantiates SerialPortManager, ASTMFramer, BS230Parser,
 ...
 *   await engine.start();
 *
 * The Mindray BS-230 uses ASTM E1394 / LIS2-A2 over RS-232 (manual section
 * 8.6.2). With "Send Complete Samples" enabled on the analyser, results
 * push automatically when each sample finishes.
 */
...
class IntegrationEngine {
  ...
  _initialiseModules() {
    ...
    this._serialManager.on('connected', () => {
      logger.info('Serial port connected - engine ready to receive BS-230 data');
      this._framer.reset();
    });
