 ...
 *
 * Manages the RS-232 serial connection to the Mindray BS-230 chemistry
 * analyser. Default settings per BS-230 Operator's Manual V8.0 section
 * 8.6.2 Table 8.2:
 *   Port      : COM1 (analyzer-side default; engine-side may differ)
 *   Baud rate : 9600
 *   Data bits : 8
 *   Parity    : None
 *   Stop bits : 1
...
class SerialPortManager extends EventEmitter {
  ...
  /**
   * @param {object} config
   * @param {string} config.port         - e.g. 'COM1'
   * @param {number} config.baudRate     - 9600 for BS-230
